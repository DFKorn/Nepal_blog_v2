...
const Main = () => {
  ...
  const subredditTitle = sessionStorage.getItem("subredditTitle")
    ? sessionStorage.getItem("subredditTitle")
    : "EarthPorn";
  const [selectedSubredditId, setSelectedSubredditId] =
    useState(subredditTitle);
  ...
  const selectedSubreddit = useMemo(
    () =>
      subredditsList.find((subreddit) => subreddit.id === selectedSubredditId),
    [selectedSubredditId],
  );
  ...
  return (
    <main>
      ...
      <div className="more-wrapper">
        <a
          className="more-link"
          href={`https://www.reddit.com/r/${selectedSubreddit.title}/search?q=nepal&restrict_sr=1&sr_nsfw=&sort=new`}
        >
          View More
        </a>
      </div>
